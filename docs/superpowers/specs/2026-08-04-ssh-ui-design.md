...
metadata にはスキーマバージョン、グループ、タグ、色、メモ、お気に入り、表示順を保存する。秘密鍵本文、公開鍵本文、パスフレーズ、Keychain の秘密値は保存しない。
...
各 Host は設定を継承するプライマリグループを一つだけ持つ。グループは親子階層を持ち、複数タグは metadata 上の整理情報として設定に影響させない。
