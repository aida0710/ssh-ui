...
- **更新はダウンロードしたバイトで自分自身を置き換えます。検証は署名です。** `checksums.txt` に Ed25519 の署名を併置し、**公開鍵はバイナリに焼き込んであります**。バイトの背後にあるのは TLS でも公開したアカウントでもなく、この鍵です。GitHub アカウントを取った相手は何でも公開できますが、**焼き込まれた鍵が検証できる署名は作れない**ので、どのインストールもそれを受け取りません。
- チェックサム自体は転送の破損しか見ていません（バイナリと同じ場所から来るため）。意味を持つのは、それを載せたファイルが署名されているからです。**マニフェストに署名し、マニフェストが成果物を担保する**という構図です。
- **署名の無いリリースは拒否します**（警告付きで受け入れたりしません）。受け入れてしまうと、攻撃者は署名ファイルを置かないだけで済みます。鍵を持たないビルドも同様に全て拒否します。
...
- ソースから使っている場合は `make update`（`git pull --ff-only` + `make install`）です。こちらはリリースのバイナリを取りません。
