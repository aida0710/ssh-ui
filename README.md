...
- **更新はダウンロードしたバイトで自分自身を置き換えます。** 検証しているのはリリースに併置された `checksums.txt` との SHA-256 一致だけで、これは**転送の破損を検出するもの**です。チェックサムはバイナリと同じ場所から来るので、その場所が正直かどうかについては何も言いません。**バイトの背後にあるのは TLS と、公開した GitHub アカウントだけです。** つまり、そのアカウントが乗っ取られれば、全ての秘密鍵の隣に居るアプリケーションへ攻撃者のコードが配られます。署名（公開鍵をバイナリに焼き込む方式）で塞げますが、現時点では入れていません。これは意識した上での選択です。
