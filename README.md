...
- **このアプリが自分以外のホストへ通信するのは、更新確認のここだけです。** `https://api.github.com/repos/aida0710/ssh-ui/releases/latest` へ 1 回 GET します。要求はサーバー側から出すので、ページの `connect-src` は `'self'` のままで、「他のオリジンへ一切リクエストを出さない」という e2e も成立し続けます。自動では走らず、画面を開いたときだけです。
