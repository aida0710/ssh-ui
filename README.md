...
- **cookie だけでは何の証明にもなりません。** cookie はポートを区別せず、`SameSite` の「サイト」判定にもポートは入らないため、同じブラウザで `http://127.0.0.1:<別ポート>` を開くと session cookie はそちらへも送られます。そこで、読み取りを含む全ての API 要求に `X-SSH-UI-CSRF` を要求します。このトークンはページのメモリにしか無く、別ポートへは決して送られません。例外は `POST /api/v1/session/renew`（リロード直後はトークンが無く、これがトークンを取り戻す手段そのものであるため）と `GET /api/v1/health`。
...
- API は同一オリジンのみです。CORS は有効化せず、**全ての `/api/` 要求が**（読み取りも含めて）`X-SSH-UI-CSRF` header を要求し、`/api/` 応答は `Cache-Control: no-store` を返します。エラー応答は安定コードと位置情報のみを含み、設定本文は返しません（利用者が解決すべき競合差分を除く）。
...
- **このアプリが自分以外のホストへ通信するのは、更新確認のここだけです。** `https://api.github.com/repos/aida0710/ssh-ui/releases/latest` へ 1 回 GET します。要求はサーバー側から出すので、ページの `connect-src` は `'self'` のままで、「他のオリジンへ一切リクエストを出さない」という e2e も成立し続けます。自動では走らず、画面を開いたときと更新ボタンを押したときだけです。
- **自分自身を置き換えられるのは、タグから作られたビルドだけです。** 作業ツリーからのビルドは署名鍵を一切持たないので、何も検証できず、何も提供しません（画面は「`make update` を使ってください」と言います）。目の前の人がたった今ビルドした成果物を、そのビルド自身がネットワーク越しの何かで置き換えるべきではないためです。
- **検証は署名です。** `checksums.txt` に Ed25519 の署名を併置し、**公開鍵はリリースビルドに焼き込んであります**。バイトの背後にあるのは TLS でも公開したアカウントでもなく、この鍵です。GitHub アカウントを取った相手は何でも公開できますが、焼き込まれた鍵が検証できる署名は作れないので、どのインストールもそれを受け取りません。
- チェックサム自体は転送の破損しか見ていません（バイナリと同じ場所から来るため）。意味を持つのは、それを載せたファイルが署名されているからです。**マニフェストに署名し、マニフェストが成果物を担保する**という構図です。
- **署名の無いリリースは拒否します**（警告付きで受け入れたりしません）。受け入れてしまうと、攻撃者は署名ファイルを置かないだけで済みます。鍵を持たないビルドも同様に全て拒否します。署名の検証はバイナリを取りに行く**前**なので、署名されていないリリースは小さいダウンロード 1 回で止まります。
- 新しいファイルは同じディレクトリに書いてから rename で置き換えるので、途中で失敗しても動くバイナリはそのまま残ります。**実行中のプロセスは古いコードのまま動き続けます**（ファイルを置き換えてもメモリ上のものは置き換わりません）。画面は「再起動してください」と言います。
- push のたびに `snapshots/` に日付付きコピーが増えるのと同じで、**古いリリースは GitHub に残り続けます**。これは意図どおりで、手でダウンロードして検証する経路が常にあるということです。

### 署名鍵

**秘密鍵は GitHub Actions の `RELEASE_SIGNING_KEY` にしかありません。** 公開鍵は `cmd/ssh-ui/main.go` の `releaseSigningKeys` にあり、git で追跡されているので誰でも監査できます。開発ビルドはこれを使いません。

**秘密鍵を失うと、既にインストールされたバイナリはどの新しいリリースも受け取れなくなります。** その場合の置き換えは更新ではなく、最初と同じ経路での新しいバイナリの配布になります。鍵はバックアップしてください。

**交換の順番。** インストール済みのバイナリは、ビルド時に焼き込まれた鍵しか信用しません。`releaseSigningKeys` が配列なのは、この順番を成立させるためだけです。

*最初のリリースより前*なら、鍵を差し替えて Actions の secret を入れ替えるだけです。固定している相手が存在しません。

*リリースを 1 つでも出した後*は 2 回に分けます。

1. `releaseSigningKeys` に**新しい鍵を追加**する（古い方は残す）。この状態を**古い鍵で署名して**リリースする。既存のインストールは古い鍵で検証できるので受け取れ、受け取った時点で新旧どちらの署名も受け入れるバイナリになる
2. 十分に行き渡ったら、`releaseSigningKeys` から**古い鍵を削除**し、Actions の secret を新しい鍵に差し替えて、**新しい鍵で署名して**リリースする

手順 1 を飛ばして鍵を差し替えると、既存のインストールは以降どのリリースも受け取れなくなり、手で置き換える以外になくなります。**この失敗は静かに起きます** — 更新が拒否されるだけで、リリース側からは何も見えません。

- **インストールは `make install`** です。`~/.local/bin/ssh-ui` へ入れます（sudo 不要）。1 箇所に固定するのはこのアプリでは特に効きます。`SSH_ASKPASS` と Terminal 起動は実行時のバイナリの絶対パスをその場で埋め込むため、リポジトリを移動したり別チェックアウトでビルドすると保存済みパスワードでの起動が静かに壊れます。`make uninstall` で戻せます。
- **ログイン時起動は既定オフで、画面の「秘密」から切り替えます。** オンにすると `~/Library/LaunchAgents/com.github.aida0710.ssh-ui.plist` を書いて `launchctl bootstrap` します。エージェントは `-open=false` で起動し、**標準出力をどこにもリダイレクトしません**。あの出力には有効な bootstrap トークン付きの URL が乗るので、ログファイルは置き場所として不適切です。代わりに `ssh-ui open` がその場で新しい入り口を発行してブラウザを開きます。常駐は施錠された状態で始まるので、最初に一度マスターパスワードを入れる必要があります。
- **`ssh-ui <接続先>` でターミナルから直接繋げます。** 起動中の本体が `~/.ssh/ssh-ui/cli`（0600）に URL と、その起動限りの秘密を書き、このコマンドがそれを読んで一度限りの askpass トークンを貰い、環境を整えて `ssh` を exec します。本体が起動していなければ普通の `ssh` として繋ぎます（OpenSSH がパスワードを尋ねます）。このファイルを読める相手は既に vault の暗号文も全ての秘密鍵も読めるので、信頼境界は動きません。プロセスが強制終了して残骸が残っても、指す先のポートには誰も居らず、秘密は次の起動で作り直されます。
- Terminal 起動もこのコマンドを実行します。以前は 5 つの環境変数と一度限りのトークンを**そのままウィンドウのコマンド行に書いて**いたため、トークンがシェルの履歴と Terminal のスクロールバックに残っていました。今はどこにも表示されません。
